...
def fdtd_1d_solver(initial_condition, xE, dt, Tf, bounds):
...
def test_fdtd_pmc_conditions():
    ...
    initial_condition = gaussian_pulse(xE, x0, sigma)
    final_condition = fdtd_1d_solver(initial_condition, xE, dt, Tf,bounds=('pmc', 'pmc'))
...
dx = xE[1] - xE[0]
dt = 0.5 * dx / C0  # Condición de estabilidad CFL
Tf =np.max(xE) - np.min(xE)/ C0 # Tiempo final de simulación


initial_condition = gaussian_pulse(xE, x0, sigma)
#fdtd_1d_solver(initial_condition, xE, dt, Tf, bounds=('pmc', 'pmc'))
test_fdtd_pmc_conditions()